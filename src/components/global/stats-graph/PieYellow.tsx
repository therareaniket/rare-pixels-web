...
// 							<Pie key={isInView ? "in-view" : "hidden"} data={chartData} dataKey="visitors" nameKey="browser" innerRadius="50%" outerRadius="90%" paddingAngle={2} cornerRadius={2} isAnimationActive={isInView} animationDuration={2000} animationEasing="ease-out" />
// 						</PieChart>
// 					</ChartContainer>
// 				</div>
// 			</CardContent>
...
export default function PieYellow() {
  ...
  useEffect(() => {
    ...
    const trigger = ScrollTrigger.create({
      ...
      onEnter: () => {
        setIsInView(true)
        // Fade in the element smoothly using GSAP
        gsap.to(element, {
          opacity: 1,
          duration: 0.6,
          ease: "power2.out",
  ...
  return (
    <Card className="impact-1 flex flex-col">
      <CardContent className="flex-1 pb-0 flex items-center justify-center">
        {/* Set initial opacity-0 via CSS/Tailwind */}
